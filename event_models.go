...
func getAttendees(event Event) ([]string, error) {
	var attendees []string
	query := `SELECT name FROM attendees WHERE eventid=$1`
...
func addAttendee(event Event, name string) (int, error) {
	addStatement := `
		INSERT INTO attendees (eventid, name)
		VALUES ($1, $2)
		RETURNING id;
	`
	...
	// Use the global db object
	newID := 0
	err := db.QueryRow(addStatement, event.ID, name).Scan(&newID)
	return newID, err
}
